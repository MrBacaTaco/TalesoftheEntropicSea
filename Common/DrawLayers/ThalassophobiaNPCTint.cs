...
{
    public class ThalassophobiaNPCTint : GlobalNPC
    {
        ...
        {
            if (Main.dedServ || !npc.active) return;
            ...
            Vector2 globalWobble = new Vector2((float)Math.Sin(t * 4f + npc.position.Y * 0.02f) * 2f, 0f);
            Vector2 basePos = npc.Center - screenPos + new Vector2(0f, npc.gfxOffY);

            for (int k = 0; k < LAYERS; k++)
            {
                int seed = Hash3(npc.whoAmI, npc.frame.Y, k);
                ...
                Vector2 pos = basePos + globalWobble + drift;

                spriteBatch.Draw(tex, pos, frame, tint, npc.rotation, origin, npc.scale * scaleMul, fx, 0f);
            }
        }
        ...
        static float Frac(int x) => (x & 0x7FFFFFFF) / 2147483647f;
